...
import type { ClassBuilder, EnumBuilder } from '@boundaryml/baml/type_builder'
...
export default class TypeBuilder {
  ...
  constructor() {
    ...
    this.DummyOutput = this.tb.classBuilder('DummyOutput', ['nonce', 'nonce2'])

    this.DynInputOutput = this.tb.classBuilder('DynInputOutput', ['testKey'])

    this.DynamicClassOne = this.tb.classBuilder('DynamicClassOne', [])

    this.DynamicClassTwo = this.tb.classBuilder('DynamicClassTwo', ['hi', 'some_class', 'status'])

    this.DynamicOutput = this.tb.classBuilder('DynamicOutput', [])

    this.OriginalB = this.tb.classBuilder('OriginalB', ['value'])

    this.Person = this.tb.classBuilder('Person', ['name', 'hair_color'])

    this.SomeClassNestedDynamic = this.tb.classBuilder('SomeClassNestedDynamic', ['hi'])

    this.Color = this.tb.enumBuilder('Color', ['RED', 'BLUE', 'GREEN', 'YELLOW', 'BLACK', 'WHITE'])

    this.DynEnumOne = this.tb.enumBuilder('DynEnumOne', [])

    this.DynEnumTwo = this.tb.enumBuilder('DynEnumTwo', [])

    this.Hobby = this.tb.enumBuilder('Hobby', ['SPORTS', 'MUSIC', 'READING'])
  }
