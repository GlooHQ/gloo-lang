...
pub fn full_document_range(document: &TextDocumentItem) -> Range {
    // Compute the number of lines. If the text is empty, assume one line.
    let line_count = if document.text.is_empty() {
        1
    } else {
...
pub fn get_current_line<'a>(document: &'a TextDocumentItem, line: u32) -> &'a str {
    document.text.lines().nth(line as usize).unwrap_or("")
}
...
pub fn get_word_at_position(document: &TextDocumentItem, position: &Position) -> String {
    let current_line = get_current_line(document, position.line);
    ...
    let end = if let Some(mat) = re_end.find(substring_after) {
        ...
        // Mimic the TS behavior: if no non-word character is found, return empty string.
        return "".to_string();
    };
...
pub fn get_symbol_before_position(document: &TextDocumentItem, position: &Position) -> String {
