...
export const ResponseRenderer: React.FC<ResponseRendererProps> = ({
  ...
  const shouldShowParsedSeparately = () => {
    ...
    return (
      parsedValue &&
      JSON.stringify(JSON.parse(parsedValue)) !==
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        className={`grid ${
          shouldShowParsedSeparately() ? "grid-cols-2" : "grid-cols-1"
        } gap-4`}
      >
        {/* LLM Response */}
        {llmResponse && (
      ...
      {/* Error Messages */}
      {failureMessage && (
        <div className="text-xs text-red-500">Error: {failureMessage}</div>
      )}
    </div>
  );
};
...
const MetadataBadges: React.FC<{ llmResponse: WasmLLMResponse }> = ({
  llmResponse,
}) => (
  <TooltipProvider>
    <Tooltip>
